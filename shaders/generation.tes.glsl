...
out vec4 TENormal;   // Vertex normal in camera coords.
...
void main() {
    ...
    mat4 MVP = projectionMtx * viewMtx * modelMtx;

    // Transform to clip coordinates
    gl_Position = MVP * TEPosition;
    // Convert to camera coordinates
    TEPosition = MVP * TEPosition;
    cameraVec = normalize( -(viewMtx * modelMtx*vec4(TEPosition)).xyz );
    TENormal = vec4(normalize(normalMtx * vec4(n,0)));
    TELight = normalize( (viewMtx * vec4(lPos,1.0)).xyz - (viewMtx * modelMtx*TEPosition).xyz );
    halfwayVec = normalize( cameraVec + TELight);
}